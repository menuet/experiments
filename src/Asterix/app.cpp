...
public:
    Impl(sdlxx::Window&& window, sdlxx::Renderer&& renderer, World&& world)
        : m_window(std::move(window)), m_renderer(std::move(renderer)), m_world(std::move(world))
    {
    }

    void run()
    {
        const auto event_poller = sdlxx::make_event_poller(sdlxx::on<SDL_QUIT>([](const auto&) {}),
...
{
    EXP_BOOST_OUTCOME_TRY(window, sdlxx::create_window("Hello World!", SCREEN_SIZE));

    EXP_BOOST_OUTCOME_TRY(renderer, sdlxx::create_renderer(window));

    EXP_BOOST_OUTCOME_TRY(world, load_world(renderer));

    auto app_impl = std::make_unique<App::Impl>(std::move(window), std::move(renderer), std::move(world));
