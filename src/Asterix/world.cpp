...
static sdlxx::result<sdlxx::Texture> format_number_to_texture(const sdlxx::Renderer& renderer, const sdlxx::Font& font,
                                                              const char* name, int number)
{
    const auto number_string = stdnext::format("{} {}", name, number);
    return sdlxx::create_text_texture(renderer, font, number_string.c_str(), sdlxx::ColorAlpha{255, 255, 255, 255});
}

class Forest
{
public:
    Forest(const sdlxx::Renderer& renderer, sdlxx::Texture&& texture, sdlxx::Font&& font) noexcept
        : m_texture{std::move(texture)}, m_font(std::move(font)),
          m_miams_obelix_texture{format_number_to_texture(renderer, m_font, "Obelix", m_miams_obelix)},
          m_miams_asterix_texture{format_number_to_texture(renderer, m_font, "Asterix", m_miams_asterix)}
    {
    }
    ...
    void render(const sdlxx::Renderer& renderer) const noexcept
    {
        ...
        if (m_miams_obelix_texture)
            sdlxx::render_texture(
                ...
                sdlxx::Point{SCREEN_SIZE.w() - sdlxx::get_size(m_miams_obelix_texture.value()).w() - FONT_SIZE,
                             FONT_SIZE});
        if (m_miams_asterix_texture)
            sdlxx::render_texture(renderer, m_miams_asterix_texture.value(), sdlxx::Point{FONT_SIZE, FONT_SIZE});
    }
...
public:
    ...
    {
        std::uniform_int_distribution<> x_distrib(0, SCREEN_SIZE.w() - m_size.w());
        std::uniform_int_distribution<> y_distrib(0, SCREEN_SIZE.h() - m_size.h());
        for (std::size_t boar_index = 0; boar_index < boars_count; ++boar_index)
            m_boars_locations.push_back(sdlxx::Point{x_distrib(random_generator), y_distrib(random_generator)});
    }
    ...
    bool are_colliding(const sdlxx::Rectangle& other) noexcept
    {
        const auto collided_boar_iter =
            std::find_if(begin(m_boars_locations), end(m_boars_locations), [&](const auto& location) {
                return sdlxx::are_colliding(sdlxx::Rectangle{location, m_size}, other);
            });
...
public:
    Obelix(sdlxx::Texture&& texture, sdlxx::Chunk&& miam_chunk) noexcept
        : m_texture{std::move(texture)}, m_miam_chunk{std::move(miam_chunk)}, m_size{sdlxx::get_size(this->m_texture)}
    {
    }

    void move() noexcept
    {
        const auto next_location = sdlxx::Point(m_location.x() + m_direction.w() * ANIMATION_STEP.w(),
                                                m_location.y() + m_direction.h() * ANIMATION_STEP.h());

        bool ok_to_move = true;
        if (next_location.x() < 0 || next_location.x() + m_size.w() > SCREEN_SIZE.w())
        {
            m_direction = sdlxx::Size(-m_direction.w(), m_direction.h());
            ok_to_move = false;
        }
        if (next_location.y() < 0 || next_location.y() + m_size.h() > SCREEN_SIZE.h())
        {
            m_direction = sdlxx::Size(m_direction.w(), -m_direction.h());
            ok_to_move = false;
        }
...
{
public:
    Asterix(sdlxx::SpriteSheet&& sprite_sheet, sdlxx::Chunk&& miam_chunk, sdlxx::Chunk&& aie_chunk) noexcept
        : m_sprite_sheet{std::move(sprite_sheet)}, m_miam_chunk{std::move(miam_chunk)},
    ...
    void move() noexcept
    {
        ...
        const auto next_location = sdlxx::Point(m_location.x() + direction().w() * ANIMATION_STEP.w(),
                                                m_location.y() + direction().h() * ANIMATION_STEP.h());
        if (next_location.x() >= 0 && next_location.x() + sprite_size.w() <= SCREEN_SIZE.w() &&
            next_location.y() >= 0 && next_location.y() + sprite_size.h() <= SCREEN_SIZE.h())
            m_location = next_location;
    ...
    {
        m_keys = static_cast<Keys>(static_cast<int>(m_keys) | static_cast<int>(key));
    }
...
World::World(std::unique_ptr<Forest>&& forest, std::unique_ptr<Boars>&& boars, std::unique_ptr<Obelix>&& obelix,
             std::unique_ptr<Asterix>&& asterix)
    : m_forest(std::move(forest)), m_boars(std::move(boars)), m_obelix(std::move(obelix)), m_asterix(std::move(asterix))
{
}
...
void World::set_key(Keys key) noexcept
{
    m_asterix->set_key(key);
}

void World::unset_key(Keys key) noexcept
{
    m_asterix->unset_key(key);
}
...
void World::move(const sdlxx::Renderer& renderer) noexcept
{
    ...
    const auto obelix_touched_asterix = sdlxx::are_colliding(m_obelix->rectangle(), m_asterix->rectangle());
...
{
    EXP_BOOST_OUTCOME_TRY(forest_texture, sdlxx::load_texture(renderer, sdlxx::get_asset_path("Forest.bmp")));

    EXP_BOOST_OUTCOME_TRY(forest_font, sdlxx::load_font(sdlxx::get_asset_path("leadcoat.ttf"), FONT_SIZE));

    auto forest = std::make_unique<Forest>(renderer, std::move(forest_texture), std::move(forest_font));

    EXP_BOOST_OUTCOME_TRY(boars_texture, sdlxx::load_texture(renderer, sdlxx::get_asset_path("Boar.Sprite.bmp"),
                                                             sdlxx::Color{52, 80, 225}));

    auto boars = std::make_unique<Boars>(std::move(boars_texture), 10);

    EXP_BOOST_OUTCOME_TRY(obelix_texture, sdlxx::load_texture(renderer, sdlxx::get_asset_path("Obelix.Sprite.bmp"),
                                                              sdlxx::Color{252, 254, 252}));

    EXP_BOOST_OUTCOME_TRY(obelix_miam_chunk, sdlxx::load_wav(sdlxx::get_asset_path("Obelix.Miam.wav")));

    auto obelix = std::make_unique<Obelix>(std::move(obelix_texture), std::move(obelix_miam_chunk));

    EXP_BOOST_OUTCOME_TRY(asterix_sprite_sheet,
                          sdlxx::load_spritesheet(renderer, sdlxx::get_asset_path("Asterix.Sprites.bmp"),
                                                  sdlxx::Color{255, 200, 0}, sdlxx::Size{48, 64}));

    EXP_BOOST_OUTCOME_TRY(asterix_miam_chunk, sdlxx::load_wav(sdlxx::get_asset_path("Asterix.Miam.wav")));

    EXP_BOOST_OUTCOME_TRY(asterix_aie_chunk, sdlxx::load_wav(sdlxx::get_asset_path("Asterix.Aie.wav")));

    auto asterix = std::make_unique<Asterix>(std::move(asterix_sprite_sheet), std::move(asterix_miam_chunk),
                                             std::move(asterix_aie_chunk));

    return World(std::move(forest), std::move(boars), std::move(obelix), std::move(asterix));
}
