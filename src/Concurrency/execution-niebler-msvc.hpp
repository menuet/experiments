...
namespace std::execution {
  ...
  namespace __set_value {
    ...
    // clang-format off
    template <typename R, typename... As> concept has_member_set_value = requires(R&& r, As&&... as) { ((R&&)r).set_value((As&&) as...); };
    template <typename R, typename... As> concept has_free_set_value = requires(R&& r, As&&... as) { set_value((R&&)r, (As&&) as...); };
    // clang-format on
    ...
    struct __fn {
      struct __impl {
        ...
        auto operator()(R&& r, As&&... as) const {
            if constexpr (has_member_set_value<R, As...>) {
            ((R&&)r).set_value((As&&) as...);
            return __result<true, noexcept(((R&&)r).set_value((As&&) as...))>{};
          } else if constexpr (has_free_set_value<R, As...>) {
            set_value((R&&)r, (As&&) as...);
  ...
  namespace __set_error {
    ...
    // clang-format off
    template <typename R, typename E> concept has_member_set_error = requires(R&& r, E&& e) { ((R &&) r).set_error((E &&) e); };
    template <typename R, typename E> concept has_free_set_error = requires(R&& r, E&& e) { set_error((R &&) r, (E &&) e); };
    // clang-format on
    ...
    struct __fn {
      struct __impl {
        ...
        auto operator()(R&& r, E&& e) const {
          if constexpr (has_member_set_error<R, E>) {
            ((R&&)r).set_error((E&&) e);
            return __result<true, noexcept(((R&&)r).set_error((E&&) e))>{};
          } else if constexpr (has_free_set_error<R, E>) {
            set_error((R&&)r, (E&&) e);
  ...
  namespace __set_done {
    ...
    // clang-format off
    template <typename R> concept has_member_set_done = requires(R&& r) { ((R &&) r).set_done(); };
    template <typename R> concept has_free_set_done = requires(R&& r) { set_done((R &&) r); };
    // clang-format on
    ...
    struct __fn {
      struct __impl {
        ...
        auto operator()(R&& r) const {
          if constexpr (has_member_set_done<R>) {
            ((R&&)r).set_done();
            return __result<true, noexcept(((R&&)r).set_done())>{};
          } else if constexpr (has_free_set_done<R>) {
            set_done((R&&)r);
  ...
  namespace __submit {
    ...
    // clang-format off
    template <typename S, typename R> concept has_member_submit = requires(S&& s, R&& r) { ((S &&) s).submit((R &&) r); };
    template <typename S, typename R> concept has_free_submit = requires(S&& s, R&& r) { submit((S &&) s, (R &&) r); };
    // clang-format on
    ...
    struct __fn {
      struct __impl {
        ...
        auto operator()(S&& s, R&& r) const {
            return 0;
          //if constexpr (has_member_submit<S, R>) {
          //  ((S&&) s).submit((R&&) r);
          //  return __result<true, noexcept(((S&&) s).submit((R&&) r))>{};
          //} else if constexpr (has_free_submit<S, R>) {
          //  submit((S&&) s, (R&&) r);
  ...
  namespace __execute {
    ...
    // clang-format off
    template <typename E, typename F> concept has_member_execute = requires(E&& e, F&& f) { ((E &&) e).execute((F &&) f); };
    template <typename E, typename F> concept has_free_execute = requires(E&& e, F&& f) { execute((E &&) e, (F &&) f); };
    // clang-format on

    struct __fn::__impl {
      template<class E, class F>
      auto operator()(E&& e, F&& f) const {
        if constexpr (has_member_execute<E, F>) {
          ((E&&) e).execute((F&&) f);
          return __result<true, noexcept(((E&&) e).execute((F&&) f))>{};
        } else if constexpr (has_free_execute<E, F>) {
          execute((E&&) e, (F&&) f);
