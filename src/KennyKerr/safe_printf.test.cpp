    ...
    namespace {
        ...
        public:
            ...
            bool isEqualTo(const char (&string)[StringSizeV]) const
            {
                ...
                return std::all_of(begin(m_array) + StringSizeV, end(m_array),
                                   [](char c) { return c == INITIAL_CHAR; });
            }
        ...
        template <size_t ArgsStringsArraySizeV>
        ArgsStringsHolder<ArgsStringsArraySizeV> make_args_strings_holder(
            const std::array<const char*, ArgsStringsArraySizeV>& argsStrings)
        {
        ...
        struct ResultHolder
        {
            ...
            ResultHolder(ArrayType& expectedBuffer) : m_pExpectedBuffer(&expectedBuffer)
            {
            }
        ...
        void call_safe_sprintf(BufferT& buffer, const char* format, const ResultHolderT& resultHolder,
                               const ArgsStringsHolderT& argsStringsHolder, const ArgTs&... args)
        {
...
#define THEN_RESULT_IS(expectedBuffer) make_result_holder(expectedBuffer)
    ...
    SCENARIO("safe_sprintf can be called with 1 parameter", "[printf]")
    {
        ...
        GIVEN("a buffer of 30 characters")
        {
            ...
            // TODO: fix 64 bits
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "p:%p", WITH_ONE_ARG((void*) 0x12345678),
            // THEN_RESULT_IS("p:12345678"));
        }
    }
    ...
    SCENARIO("safe_sprintf can be called with 2 parameters", "[printf]")
    {
        ...
        GIVEN("a buffer of 30 characters")
        {
            ...
            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "he%f-%s++", WITH_TWO_ARGS(12345.5, "BOO"),
                                        THEN_RESULT_IS("he12345.500000-BOO++"));

            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%+010d!!!%i", WITH_TWO_ARGS(42, 42),
                                        THEN_RESULT_IS("+000000042!!!42"));

            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, " %i!!!0x%08X", WITH_TWO_ARGS(42, 0x123),
                                        THEN_RESULT_IS(" 42!!!0x00000123"));
            ...
            // TODO: fix
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%e.%u.", WITH_TWO_ARGS(1234., 1234),
            // THEN_RESULT_IS("1.234000e+003.1234."));

            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%f**%.1f**", WITH_TWO_ARGS(23.45, 23.45),
                                        THEN_RESULT_IS("23.450000**23.4**"));

            // TODO: fix
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%e**%llu**", WITH_TWO_ARGS(23.45, 123LLU),
            // THEN_RESULT_IS("2.345000e+001**123**"));

            // TODO: fix
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%E**%.3f**", WITH_TWO_ARGS(23.45, 987.12),
            // THEN_RESULT_IS("2.345000E+001**987.120**"));

            // TODO: fix
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%a/he%010.10sx", WITH_TWO_ARGS(23.45, "MIAM"),
            // THEN_RESULT_IS("0x1.773333p+4/he000000MIAMx"));

            // TODO: fix
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%A/hex%xAsterix%%", WITH_TWO_ARGS(23.45, 0x987),
            // THEN_RESULT_IS("0X1.773333P+4/hex987Asterix%"));

            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "???%g|%g???", WITH_TWO_ARGS(23.45, 98.6543),
                                        THEN_RESULT_IS("???23.45|98.6543???"));

            WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "%s=%G", WITH_TWO_ARGS("result", 23.45),
                                        THEN_RESULT_IS("result=23.45"));

            // TODO: fix 64 bits
            // WHEN_SAFE_SPRINTF_IS_CALLED(buffer, "p:%p-%08x", WITH_TWO_ARGS((void*) 0x12345678, 0xABCDEF),
            // THEN_RESULT_IS("p:12345678-00abcdef"));
        }
    }
