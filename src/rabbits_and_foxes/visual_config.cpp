...
namespace nlohmann {
    ...
    template <>
    struct adl_serializer<raf::visual::Config>
    {
        static raf::visual::Config from_json(const json& j)
        {
            return raf::visual::Config{
                j.at("board_size").get<sdlxx::Size>(),
                j.at("border_size").get<sdlxx::Size>(),
                j.at("cell_size").get<sdlxx::Size>(),
                j.at("boards").get<std::map<std::string, raf::Board>>()};
        }

        static void to_json(json& j, const raf::visual::Config& config)
        {
